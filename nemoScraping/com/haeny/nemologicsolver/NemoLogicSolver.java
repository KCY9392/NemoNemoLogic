...
public class NemoLogicSolver {
	...
	private final char EMPTY = '0', FILLED = '1';
	
	private int[][] columnValue, columnIndex;
	private long[] mask, val;
	private char[][] result;
	private com.haeny.nemologicsolver.saveResult sr;
	 ...
	 * @param columnHints
	 */
	public NemoLogicSolver(int[][] rowsHints, int[][] columnHints,String title) {
		...
		this.result=new char[rowsHints.length][columnHints.length];
		this.sr=new saveResult();
		this.title=title;
	}
	...
	public void process() throws Exception {
		...
		if (dfs(0)) {
			for (int r = 0; r < ROW_SIZE; r++) {
				for (int c = 0; c < COLUMN_SIZE; c++) {
					...
					result[r][c]=(grid[r] & (1L << c)) == 0 ? EMPTY : FILLED;
				}
				
			}
		}else {
			System.out.println("일치하는행없음?");
		}
		System.out.println("===================");
		
		for (int i = 0; i < rowsHints.length; i++) {
		...
		sr.saveQuestion(result, rowsHints, columnHints,title);
	}
